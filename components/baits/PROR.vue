  ...
  <div class="grid grid-cols-12">
    <div class="col-span-12 border border-gray-400">
      ...
      <div class="px-10">
        <div
          class="inline-block"
          v-for="(location, index) in data.pror.propertyLocation"
          :key="index"
        >
          {{ location
          }}{{ data.pror.propertyLocation.length - 1 === index ? '' : ',' }}
          &nbsp;
        </div>
      </div>
      <div
      ...
      >
        ...
        <div>
          ...
          <br />
          <span>{{ data.pror.type }}</span>
        </div>
        <div>
          <span class="font-bold">Property Type</span>
          <br />
          <span>{{ data.pror.propertyType }}</span>
        </div>
        <div>
          <span class="font-bold">Number of Rooms</span>
          <br />
          <span>{{ data.pror.propertyRoom }}</span>
        </div>
        <div>
          <span class="font-bold">Property size (sq ft)</span>
          <br />
          <span>
            {{ numberWithCommasHandler(data.pror.propertySize) }}
          </span>
        </div>
        <div>
          <span class="font-bold">Estimated Price</span>
          <br />
          <span>${{ numberWithCommasHandler(data.pror.price) }}</span>
        </div>
        <div>
          <span class="font-bold">Category</span>
          <br />
          <span>{{ data.pror.category }}</span>
        </div>
        <div>
          <span class="font-bold">Preferred Date</span>
          <br />
          <span>{{ data.pror.datePreferredBy }}</span>
        </div>
        <div>
          <span class="font-bold">Tenancy</span>
          <br />
          <span>{{ data.pror.propertyTenancy }}</span>
        </div>
        <div class="sm:col-span-2">
          <div class="font-bold">Remarks</div>
          <div>{{ data.pror.remarks }}</div>
        </div>
        <div>
