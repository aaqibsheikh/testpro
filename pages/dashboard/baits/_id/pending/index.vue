<template>
  <div class="w-full px-6 md:px-12 m-auto sm:mt-8 mt-32 md:mt-16">
    <BaitsHeader text="Pending Bites" />
    <div v-if="calcWidth < 1024">
      <div v-if="pending.length > 0">
        <!-- This list design for mobile screen -->
        <div class="grid grid-cols-1 border border-gray-300 px-3 py-6 mt-4 lg:hidden block" v-for="(bait, key) in pending" :key="key">
          ...
          <div class="flex flex-wrap mt-4">
            <button
              @click="acceptBite(bait.responseId)"
              class="w-full items-center cursor-pointer px-12 py-1 shadow text-xl text-white bg-baits-blue rounded-full focus:outline-none"
            >
              <font-awesome-icon class="mr-2 text-white inline-block w-4 h-4" :icon="['fas', 'check']" />
              <span>Accept</span>
            </button>
            <button
              @click="rejectBite(bait.responseId)"
              class="w-full items-center cursor-pointer px-12 py-1 shadow text-xl text-white bg-gray-400 rounded-full focus:outline-none mt-2"
            >
      ...
      <div v-else>
        <div class="col-span-12 text-center">
          <h2 class="text-gray-800 font-bold text-xl p-20">There are no pending bites for this bait</h2>
        </div>
      </div>
    </div>
    <div v-else></div>
    <div v-if="calcWidth >= 1024">
      <div v-if="pending.length > 0">
        <div class="grid grid-cols-12 border border-gray-300 px-8 py-4" v-for="(bait, key) in pending" :key="key">
          ...
          <div class="col-start-10 col-end-12 text-center">
            <div class="w-10 cursor-pointer inline-block">
              <button
                @click="acceptBite(bait.responseId)"
                class="w-48 items-center px-12 py-1 shadow text-xl text-white bg-baits-blue rounded-full focus:outline-none"
              >
                <font-awesome-icon class="mr-2 text-white inline-block w-4 h-4" :icon="['fas', 'check']" />
                <span>Accept</span>
              </button>
              <button
                @click="rejectBite(bait.responseId)"
                class="w-48 items-center px-12 py-1 shadow text-xl text-white bg-gray-400 rounded-full focus:outline-none mt-2"
              >
                <font-awesome-icon class="mr-2 text-white inline-block w-4 h-4" :icon="['fas', 'times']" />
                <span>Reject</span>
              </button>
      ...
      <div v-else>
        <div class="col-span-12 text-center">
          <h2 class="text-gray-800 font-bold text-xl p-20">There are currently no pending bites for this bait</h2>
        </div>
      </div>
    </div>
    <div v-else></div>
    <!-- <div class="grid grid-rows-1 grid-flow-col" v-if="pending.data.listing">
      <div>
        <vue-ads-pagination :total-items="pending.total" :items-per-page="Number.parseInt(pending.page.pageSize)" :page="page" :loading="loading" @page-change="pageChange">
    ...
    <AcceptBiteForBaitModal :id="acceptResponseId" v-if="acceptBiteShow" />
    <RejectBiteForBaitModal :id="rejectResponseId" v-if="rejectBiteShow" />
  </div>
</template>
<script>
import { mapGetters, mapActions } from 'vuex';
import BaitsHeader from '@/components/baits/BaitsHeader';
import AcceptBiteForBaitModal from '@/components/modal/AcceptBiteForBaitModal';
import RejectBiteForBaitModal from '@/components/modal/RejectBiteForBaitModal';
import { calculateYears, calculatePlatformJoinDate } from '@/services/index';

export default {
  components: {
    AcceptBiteForBaitModal,
    RejectBiteForBaitModal,
    BaitsHeader,
  },
  data() {
    return {
      ...
      loading: false,
      selectItems: 5,
      acceptBiteShow: false,
      rejectBiteShow: false,
      acceptResponseId: null,
      rejectResponseId: null,
    };
  },
  async fetch({ app, params, store }) {
    await store.dispatch('baits/getPendingBaitsList', {
      pageNum: '',
      pageSize: '',
      id: params.id,
    });
  ...
  computed: {
    ...mapGetters({
      pending: 'baits/getPendingBaitsList',
    }),
    calcWidth() {
  ...
  mounted() {
    // this.page = Number.parseInt(this.pending.page.pageNum) - 1;
    this.acceptBiteShow = true;
    this.rejectBiteShow = true;
  },
  methods: {
    ...mapActions({
      getPendingBaitsList: 'baits/getPendingBaitsList',
      rejectBiteForBait: 'baits/rejectBiteForBait',
    }),
    ...
    pageChange(page) {
      ...
      this.getPendingBaitsList({
        pageNum: this.page,
        pageSize: this.pending.page.pageSize,
    ...
    acceptBite(responseId) {
      this.acceptResponseId = responseId;
      VoerroModal.show('accept-bite-for-bait-modal');
    },
    rejectBite(responseId) {
      this.rejectResponseId = responseId;
      VoerroModal.show('reject-bite-for-bait-modal');
    },
  },
};
</script>
