        ...
        <div class="flex flex-wrap bg-white -mx-3 p-4 mt-2 rounded-sm">
          <div class="lg:w-1/2 md:w-4/6 sm:w-4/5 w-full px-3">
            ...
            <div class="flex flex-wrap items-start mb-4">
              <div>
                <label class="inline-flex items-center mt-3 mr-6">
                  <input type="checkbox" name="featured" v-model="blog.isFeatured" class="form-checkbox h-5 w-5 text-gray-600" checked />
                  <span class="ml-2 text-gray-700">Featured</span>
                </label>
              </div>
              <div>
                <label class="inline-flex items-center mt-3 mr-6">
                  <input type="checkbox" name="popular" v-model="blog.isPopular" class="form-checkbox h-5 w-5 text-red-600" checked />
                  <span class="ml-2 text-gray-700">Popular</span>
                </label>
              </div>
              <div>
                <label class="inline-flex items-center mt-3 mr-6">
                  <input type="checkbox" name="recent" v-model="blog.isRecent" class="form-checkbox h-5 w-5 text-orange-600" checked />
                  <span class="ml-2 text-gray-700">Recent</span>
                </label>
              </div>
            </div>
            <div class="flex flex-wrap -mx-2">
            ...
            <div class="flex flex-wrap -mx-2">
              <div class="w-full mb-4 px-2">
                ...
                <v-select v-model="initStatus" :options="status" label="text" :clearable="false" v-validate="'required'" data-vv-name="status">
                  <template #search="{ attributes, events }">
                    <input class="vs__search" v-bind="attributes" v-on="events" />
                  </template>
                </v-select>
          ...
          <div class="lg:w-1/2 md:w-4/6 sm:w-4/5 w-full px-3">
            <div class="flex flex-wrap -mx-2">
              <div class="w-full px-2 mb-4">
                <label class="block tracking-wide text-gray-700 text-sm font-bold mb-2">Image Gallery</label>
                <client-only>
                  <image-uploader
                    :maxWidth="500"
                    :maxHeight="400"
            ...
            <div class="flex flex-wrap -mx-2">
              <div class="w-full px-2 mb-4">
                ...
                >
                  ...
                  <p v-if="thumbnailImages.length === 0 && blog.thumbnails.length === 0" class="py-4">Click to upload image</p>
                </div>
                <div class="validate-error-msg text-red-500 text-sm">
  ...
  methods: {
    ...
    setStatus() {
      this.status.find((item) => {
        if (item.text === this.blog.status) this.initStatus = item;
      });
    },
        ...
        if (result) {
          ...
          Promise.all(promises).then(async () => {
            ...
            const postData = {
              ...
              featured: this.blog.isFeatured,
              popular: this.blog.isPopular,
              recent: this.blog.isRecent,
              summary: summaryText,
            };
            ...
            this.$axios
              ...
              .then((response) => {
                if (response.data.result) {
                  ...
                  // listing
                  this.$router.push(`/dashboard/admin/blog`);
