  ...
  <div class="w-full m-auto sm:mt-8 mt-32 md:mt-16">
    <div class="grid grid-cols-2 px-5 md:px-12">
      <div class="text-lg col-span-12 sm:col-span-1 sm:mb-0 mb-3">
        ...
        <span class="font-bold text-baits-blue">Closed Bites</span>
      </div>
    ...
    <div class="grid grid-cols-1">
      ...
      <div class="grid grid-cols-1 mt-5 mb-2 px-5 md:px-12">
        <div>
          <span class="text-gray-700 font-bold text-xl">Closed Bites</span>
        </div>
      </div>
      <template v-if="closedBites.data.listing.length > 0">
        <div class="col-span-12 sm:text-left text-center px-0 md:px-12">
          <!-- Desktop list design -->
          <div class="sm:block hidden">
            <div
              class="border-2 flex flex-wrap"
              :class="[index === 0 ? 'mt-3' : '', index + 1 != closedBites.data.listing.length ? 'border-b-0' : '']"
              v-for="(bite, index) in closedBites.data.listing"
              :key="index"
            >
              <div class="px-4 py-6 xl:w-4/12 lg:w-4/12 w-4/12">
                <p @click="showBiteDetail(bite.id)" class="xl:text-xl lg:text-base text-base font-bold text-baits-blue underline cursor-pointer">
                  {{ bite.description }}
                </p>
                <p class="text-sm">
              ...
              <div class="px-4 py-6 xl:w-2/12 lg:w-2/12 w-2/12">
                <p class="xl:text-xl lg:text-base text-base text-baits-green font-bold">{{ bite.amount }}</p>
              </div>
              <div class="px-4 py-6 xl:w-3/12 lg:w-2/12 w-2/12">
                <p class="xl:text-xl lg:text-base text-base text-baits-green font-bold">Closed on {{ formatDateMonthYearHandler(bite.lastActivity) }}</p>
              </div>
            </div>
          </div>
          <!-- Mobile list design -->
          <div class="sm:hidden block mb-6">
            ...
            >
              ...
              <div class="px-4 pt-3 w-full">
                <p class="text-lg text-baits-green font-bold">Closed on {{ formatDateMonthYearHandler(bite.lastActivity) }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <template v-else>
        <div class="col-span-12 text-center">
          <h2 class="text-gray-800 font-bold text-xl p-20">There are no closed bites</h2>
        </div>
      </template>
    </div>
    <div class="grid grid-rows-1 grid-flow-col" v-if="closedBites.data.listing.length > 0">
      <div>
        <vue-ads-pagination
          :total-items="closedBites.total"
          :items-per-page="Number.parseInt(closedBites.page.pageSize)"
          :page="page"
          :loading="loading"
...
export default {
  ...
  async fetch({ store }) {
    await store.dispatch('bite/getClosedBitesList', {
      pageNum: '',
      pageSize: '',
    });
  },
  computed: {
    ...mapGetters({
      closedBites: 'bite/listClosedBites',
    }),
  },
  mounted() {
    this.isDomReady = true;
    this.page = Number.parseInt(this.closedBites.page.pageNum) - 1;
  },
  methods: {
    ...mapActions({
      getClosedBitesList: 'bite/getClosedBitesList',
      getBiteDetails: 'bite/getBiteDetails',
    }),
    pageChange(page) {
      this.page = page;
      this.getClosedBitesList({
        pageNum: this.page,
        pageSize: this.closedBites.page.pageSize,
      });
    },
    getSelectedItemsByPaging(pageSize) {
      this.getClosedBitesList({ pageNum: '', pageSize: pageSize });
    },
